...
const TypeEnv = struct {
  ...
  pub fn copy(self: *@This(), source: *@This()) void {
    self.global = source.global.copy();
...
pub const TypeChecker = struct {
  ...
  /// track narrowing
  narrowing: u32 = 0,
  /// type context from current module
  ctx: *TContext,
  /// current module object
  cmo: *tir.Module = undefined,
  /// current module type
  cmt: *Type = undefined,
  ...
  /// prelude nodes
  pub const Prelude = struct {core: *Node, module: *Type,};

  /// name, typename, token
  pub const TraitInfo = struct{[]const u8, []const u8, Token};

  pub const GenInfoMap = ds.ArrayHashMap(*Node, *ds.ArrayList(GenInfo));
  ...
  pub fn init(allocator: Allocator, diag: *Diagnostic, namegen: util.NameGen) Self {
    return .{
      ...
      .generics = GenInfoMap.init(allocator),
  ...
  fn loadPrelude(self: *Self) void {
    ...
    var typ = Type.newModule(root, ks.CoreModuleVar, self.al);
    self.cmo = typ.module();
    self.cmt = typ;
  ...
  fn makeSynthName(self: *Self, name: ?[]const u8, args: tir.TypeItems, targs: ?TypeItems, cls_ty: ?*Type) []const u8 {
  ...
  fn createFunSynthName(self: *Self, fun: *tir.BasicFunNode, args: *TypeList, targs: ?TypeItems, cls_ty: ?*Type) []const u8 {
    if (fun.data.modifier.isBuiltin()) return fun.data.name.?.lexeme();
    return self.makeSynthName(if (fun.data.name) |nm| nm.lexeme() else null, args.items(), targs, cls_ty);
  }
  ...
  inline fn findCoreType(self: *Self, name: []const u8) ?*Type {
    return self.ctx.lookupInCoreTypScope(name);
  }
  ...
  inline fn findName(self: *Self, name: []const u8) ?*Type {
    return self.ctx.lookupInVarScope(name);
  }
  ...
  fn insertType(self: *Self, var_typ: *Type, typ: *Type) void {
    self.ctx.typ_scope.insert(var_typ.variable().lexeme(), .{.typ = typ, .aspec = typ.aspec});
  }
  ...
  pub inline fn insertVar(self: *Self, name: []const u8, ty: *Type) void {
    self.ctx.var_scope.insert(name, .{.typ = ty, .aspec = ty.aspec});
  }

  pub inline fn insertTy(self: *Self, name: []const u8, ty: *Type) void {
    self.ctx.typ_scope.insert(name, .{.typ = ty, .aspec = ty.aspec});
  }

  pub inline fn updateVar(self: *Self, name: []const u8, ty: *Type) void {
    _ = self.ctx.var_scope.update(name, .{.typ = ty, .aspec = ty.aspec});
  }
  ...
  fn _synth(self: *Self,  vr: Token, value: []const u8, env: *TypeEnv) TypeCheckError!*TVarNode{
    ...
    const node = TVarNode.init(vr.tkFrom(val, .TkIdent)).box(self.al);
    self.diag.skipEntry();
  ...
  inline fn isEmptyOutgoingNodes(out_nodes: *FlowList) bool {
    ...
    if (out_nodes.len() == 1) {
      const nodes = out_nodes.itemAt(0).get().bb.items();
      for (nodes) |node| {
  ...
  fn getTagOrClassFieldLen(self: *Self, typ: ?*Type) usize {
    ...
    return if (typ) |ty| (
      if (ty.isClass()) (
        if (ty.klass().modifier.isBuiltin()) ty.klass().tparamsLen()
    ...
    for (nghbs.items()) |ngh| {
      for (ngh.node.get().bb.nodes.items()) |node| {
        if (node.isVarDecl() and (!is_direct_tst or ident.valueEql(node.NdVarDecl.name.lexeme()))) {
          ...
          if (val.isSubscript()) {
            const lexeme = val.NdSubscript.expr.NdTVar.value();
  ...
  fn buildMissingPatternTypes(self: *Self, default: *Type, ident: *TVarNode, is_direct_tst: bool) *Type {
    ...
    const al = self.al;
    const j = self.matchmeta.tests.len() - 1;
    ...
    for (0..self.matchmeta.tests.len()) |i| {
      ...
      if (tst.@"0".isBinary()) {
        ...
        if (right.isType()) {
          ...
          for (nghbs.items()) |ngh| {
            const bb = ngh.node.get().bb.nodes.items();
            for (bb) |node| {
              if (node.isVarDecl() and ident.valueEql(node.NdVarDecl.name.lexeme())) {
                ...
                const fields_len = self.getTagOrClassFieldLen(typ);
  ...
  fn flowInferCondition(self: *Self, flo: FlowNode, node: *Node) !void {
    ...
    if (isEmptyOutgoingNodes(&out_f_nodes)) {
      ...
      if (edges == 1 or exits >= 1) {
        ...
        while (itr.next()) |entry| {
          self.insertVar(entry.key_ptr.*, entry.value_ptr.*.typ);
        }
        return;
      }
    }
  ...
  inline fn getClassType(self: *Self, haystack: *Type, needle: *Type, token: Token) !*Type {
    return (try self.selectType(haystack, needle, token)).klassOrInstanceClass();
  }
  ...
  fn inferPatternx(self: *Self, pat: *Pattern, conses: *ConsList, expr_ty: *Type, al: Allocator) !*Type {
    switch (pat.variant.*) {
      .cons => |*cons| {
        switch (cons.tag) {
          .List => {
            const tmp = Type.newBuiltinGenericClass(cons.name, .TkList, al);
            var ty = try self.getClassType(expr_ty, tmp, pat.token);
            if (cons.args.len > 0) {
              const param_ty = ty.klass().getSlice()[0];
              for (cons.args) |arg| {
          ...
          .Tuple => {
            const tmp = Type.newBuiltinGenericClass(cons.name, .TkTuple, al);
            var ty = try self.getClassType(expr_ty, tmp, pat.token);
            if (cons.args.len > 0) {
              if (ty.klass().tparamsLen() == cons.args.len or cons.rested and ty.klass().tparamsLen() >= cons.args.len) {
                const typs = ty.klass().getSlice()[0..cons.args.len];
          ...
          .Map => {
            const tmp = Type.newBuiltinGenericClass(ks.MapVar, .TkMap, al);
            var ty = try self.getClassType(expr_ty, tmp, pat.token);
            const list = &cons.args[0].variant.cons;
          ...
          .Other => {
            ...
            if (typ.isClass()) {
              ...
              for (cons.args, cls.data.fields.items()) |arg, field| {
                const fty = try self.inferClsField(field);
  ...
  fn checkMatchDiagStart(self: *Self, md: *tir.MarkerNode) !*Type {
    const al = self.al;
    {
      ...
      const node = &md.payload.?.NdMatch;
      var m_expr_ty = try self.infer(node.expr);
      var expr_ty = m_expr_ty.klassOrInstanceClass();
      ...
      for (node.cases) |case| {
        ...
        self.ctx.enterScope();
        const ty = try self.inferPatternx(case.pattern, &conses, expr_ty, al);
  ...
  inline fn resolveNode(self: *Self, flo: FlowNode, node: *Node) !void {
    switch (node.*) {
      ...
      .NdBasicFun => |*fun| {
        ...
        if (flo.isEntryNode()) {
          ...
          if (!fun.isAnonymous()) {
            ...
            _ = try self.inferFun(node, null, null);
          }
        }
      },
      .NdGenericFun => {
      ...
      .NdImport => |*nd| {
        ...
        if (mod_ty.module().node != nd.program) {
          // deduplicate program nodes
          nd.program = mod_ty.module().node;
        }
        self.ctx.addModule(mod_ty, self.al);
        nd.typ = mod_ty;
        // handle entities
        if (nd.data.entities == null) {
          const id = nd.getImportName();
          mod_ty.module().env.alias = id;
          self.cmo.setIdTy(id, mod_ty, AccessSpecifier.getASpec(nd.data.public));
          self.cmo.setTy(id, mod_ty, AccessSpecifier.getASpec(nd.data.public));
        } else {
          // TODO:
          unreachable;
        }
  ...
  fn resolveUserClassType(self: *Self, typ: *Type, debug: Token) TypeCheckError!void {
    if (self.findCoreType(typ.klass().data.name)) |_ty| {
      ...
      typ.klassOrInstanceClass().klass().setAsResolved();
    }
  }
  ...
  fn resolveClassType(self: *Self, typ: *Type, debug: Token) TypeCheckError!void {
    if (typ.isClass()) {
      if (typ.klass().resolved or self.isCurrentlyResolving(typ)) {
        return;
      } else if (typ.klass().modifier.isBuiltin()) {
  ...
  fn resolveTraitType(self: *Self, typ: *Type, debug: Token, al: Allocator) !*Type {
    ...
    if (ty.isTrait()) {
      if (ty.trait().data.trait) |trait| {
        _ = try self.resolveTraitType(trait, debug, al);
      }
  ...
  fn synthInferClsType(self: *Self, user_ty: *Type, core_ty: *Type, debug: Token) !*Type {
    ...
    if (core_cls.isParameterized()) {
      ...
      const synth_name = self.makeSynthName(core_cls.data.name, tparams, null, null);
      ...
      if (!core_ty.isTupleTy()) {
        ...
        var uni = Type.newUnionBoxed(self.al);
  ...
  fn synthInferTraitType(self: *Self, user_ty: *Type, core_ty: *Type, debug: Token) !*Type {
    ...
    if (core_trt.isParameterized()) {
      ...
      const synth_name = self.makeSynthName(core_trt.data.name, tparams, null, null);
  ...
  fn inferVar(self: *Self, node: anytype, emit: bool) !*Type {
    ...
    if (typ.isFunction() and !typ.function().isParameterized()) {
      if (typ.function().data.node) |nd| {
        if (!self.isCheckedFun(nd) and !self.cycles.contains(nd, Node.eql)) {
          _ = try self.inferFun(nd, typ, null);
        }
      }
    ...
    } else if (typ.isModule() and !typ.module().resolved) {
      try self.resolveModuleType(typ);
    }
    node.typ = typ;
    return typ;
  }
  ...
  fn inferBinary(self: *Self, node: *tir.BinaryNode) !*Type {
    ...
    const ty = try self.checkBinary(node, lhsTy, rhsTy, false);
    node.typ = ty;
    return ty;
  }
  ...
  fn inferIs(self: *Self, node: *tir.BinaryNode) !*Type {
    ...
    _ = try self.checkBinary(node, lhsTy, ty, true);
  ...
  fn inferVarDecl(self: *Self, node: *tir.VarDeclNode, is_const: bool) !*Type {
    if (!node.value.isEmpty()) {
      ...
      // escape instance types into their proper class types
      ret = ret.klassOrInstanceClass();
      // check if this type is immutable and not from a const decl. If so, make it mutable
      if (!ret.mutable and !is_const) {
        ret = ret.forceClone(self.al);
        ret.mutable = true;
      }
      ...
      .NdDotAccess => |*da| {
        if (lhsTy.isFunction()) {
          if (da.lhs.getType()) |_ty| {
            if (!_ty.isModule()) {
              var ty = _ty.klassOrInstanceClass();
  ...
  fn inferSubscript(self: *Self, node: *tir.SubscriptNode, narrowing: bool) TypeCheckError!*Type {
    ...
    if (!narrowing and self.canNarrowSubscript(node)) {
      ...
      env.global.pushScope();
      const narrowed = self.synthesizeSubscript(node, &env, true) catch |e| return e;
  ...
  fn inferDeref(self: *Self, node: *tir.DerefNode, narrowing: bool) !*Type {
    ...
    if (!narrowing and self.canNarrowDeref(node)) {
      ...
      env.global.pushScope();
      const narrowed = self.synthesizeDeref(node, &env, true) catch return error.CheckError;
      if (self.inferVar(narrowed, false) catch null) |ty| {
        node.typ = ty;
        return ty;
      }
  ...
  fn inferDotAccess(self: *Self, node: *tir.DotAccessNode, ast_n: *Node, narrowing: bool) !*Type {
    ...
    if (lhs_ty.isStrTy()) {
      ...
      for (lhs_ty.union_().variants.values()) |_ty| {
        if (!_ty.isClassOrTrait()) continue;
        var is_ok = true;
      ...
      if (ty.isModule()) {
        try self.resolveModuleType(ty);
      } else {
        // FIXME: hack for modules/fy.veb
        const ty_ = ty.clone(self.al);
        const curr = self.diag.count();
        self.resolveType(ty_, node.lhs.getToken(), self.al) catch {
          self.diag.popUntil(curr);
          return ty;
        };
        ty.* = ty_.*;
      }
    ...
    if (!lhs_ty.isClass() and !lhs_ty.isInstance() and !lhs_ty.isTrait()) {
      return self.propertyAccessError(node.lhs.getToken(), lhs_ty, rhs_tok.lexeme(), true);
    }
    const cls_ty = lhs_ty.klassOrInstanceClass();
  ...
  fn validateCallArguments(self: *Self, fun_ty: *tir.Function, node: *tir.CallNode) !void {
    ...
    if (!node.variadic) {
      ...
      const va_ty = fun_ty.data.params[fun_ty.data.params.len - 1].klass().tparams.?[0];
  ...
  /// intercept @iter calls for List type
  ...
  fn inferCall(self: *Self, node: *tir.CallNode, ast_n: *Node) !*Type {
    ...
    if (!fun_ty.isParameterized()) {
      ...
      if (fun_ty.data.node) |fun_idx| {
        ...
        if (!is_cyclic and !self.isCheckedFun(fun_idx)) {
          _ = try self.inferFun(fun_idx, ty, null);
        }
      }
    ...
    const is_generic_mtd = old_fun_id.isGenericMtd();
    ...
    var typ = try self.inferFun(new_fun_id, new_fun_ty, null);
  ...
  fn inferClsCall(self: *Self, node: *tir.CallNode, ty: *Type, ast_n: *Node) TypeCheckError!*Type {
    ...
    if (node.targs) |targs| {
        ...
        for (cls.data.params.?, 0..) |tvar, tpos| {
          for (cls_params, 0..) |param, ppos| {
            if (param.typ.typeidEql(tvar)) {
              ...
              _ = try self.lookupName(tvar.variable().value, true);
            }
          }
        }
      }
      ...
      const cls_params: ?tir.ParamItems = if (init_mtd_id) |_nd| _nd.NdBasicFun.params else null;
    ...
    if (init_mtd_id != null) {
      try self.validateCallArguments(
        new_cls_ty.klass().getMethodTy(ks.InitVar).?.function(),
        node,
      );
    }
  ...
  fn inferTagCall(self: *Self, node: *tir.CallNode, ty: *Type) !*Type {
    ...
    for (node.args, 0..) |arg, i| {
      const inf_ty = try self.infer(arg);
    ...
    if (type_params) |tparams| {
      for (tparams.items()) |tparam| {
        for (fields.items(), 0..) |fd, i| {
          if (tparam.typeidEql(fd.typ)) {
            ...
            const t = inf.itemAt(i).klassOrInstanceClass();
      ...
      for (0..node.args.len) |i| {
        const tk = node.args[i].getToken();
        const inf_ty = inf.itemAt(i);
        const param_ty = blk: {
  ...
  fn createFunType(self: *Self, ast_node: *Node, ret: ?*Type) !*Type {
    ...
    var ty = Type.newFunction(ret_ty, self.cmt, self.al);
    var fun = ty.function();
  ...
  fn inferFunReturnTypePartial(self: *Self, graph: *FlowGraph) !?*Type {
    ...
    const prev_nodes = graph.exit().getPrevNeighbours().items();
    // unionify all return types at exit
    var uni = Union.init(self.al);
    for (prev_nodes) |ngh| {
        ...
        if (ngh.node.get().bb.getNonScopeDiagLast()) |last| {
          if (last.isRet()) {
            if (last.getType()) |typ| {
              ...
              if (!node.NdBasicFun.data.modifier.isBuiltin() and !ret_ty.isRelatedTo(typ, .RCAny, al)) {
                return self.errorFrom(
                  true, last,
  ...
  fn inferFun(self: *Self, node: *Node, typ: ?*Type, self_ty: ?*Type) !*Type {
    ...
    if (node.isBasicFun()) {
      ...
      if (self.modules.count() > 1) {
        ...
        self.insertTy(self.cmo.env.alias, self.cmt);
        alias = self.cmo.env.alias;
      }
      defer if (alias) |name| {
  ...
  fn createClsType(self: *Self, cls: *tir.StructNode, node: *Node) *Type {
    ...
    return Class.init(
      ...
      cls.data.params, node, false, cls.data.modifier, cls.data.public, cls.trait, self.al
    ).toType().box(self.al);
  }
  ...
  fn createTraitType(self: *Self, trt: *tir.StructNode, node: *Node) *Type {
    ...
    return Class.init(
      ...
      trt.data.params, node, false, trt.data.modifier, trt.data.public, trt.trait, self.al
  ...
  fn forbidReturnInInit(self: *Self, node: *Node) void {
    ...
    const list = self.cunit.lookupFunc(node).?.exit().getPrevNeighbours().items();
    for (list) |ngh| {
  ...
  fn inferClassPartial(self: *Self, node: *Node) !*Type {
    ...
    self.insertCoreType(cls.name.lexeme(), ty);
    self.cmo.setIdTy(cls.name.lexeme(), ty, ty.aspec);
    if (cls.data.modifier.isBuiltin()) {
  ...
  fn inferMethod(self: *Self, cls_ty: *Type, node: *Node, typ: *Type, self_ty: *Type) !*Type {
    ...
    typ.tid = 0;
    const full_ty = try self.inferFun(node, typ, self_ty);
    ...
    if (!cls.isParameterized()) {
      ...
      if (!cls.data.modifier.isBuiltin()) {
        ...
        if (cls.data.methods.len() > 0) {
          ...
          for (cls.data.methods.items(), 0..) |itm, i| {
            ...
            if (!itm.isGenericMtd()) {
              ...
              ty.klass().data.methods.items()[i] = try self.inferFun(itm, fun_ty, ty);
            } else {
        ...
        for (cls.data.methods.items(), 0..) |itm, i| {
          ...
          ty.klass().appendMethodTy(fun_ty, al);
          ty.klass().data.methods.items()[i] = try self.inferMethod(ty, itm, fun_ty, ty);
        }
        if (cls.trait) |trait| {
          const trt = try self.resolveTraitType(trait, token, al);
          ty.klass().data.trait = trt;
  ...
  fn inferTraitPartial(self: *Self, node: *Node) !*Type {
    ...
    // set the class's name to it's full type
    self.insertCoreType(trt.name.lexeme(), ty);
    self.cmo.setIdTy(trt.name.lexeme(), ty, ty.aspec);
    return ty;
  }
      ...
      if (trt.data.methods.len() > 0) {
        ...
        for (trt.data.methods.items(), 0..) |itm, i| {
          ...
          if (!itm.isGenericMtd()) {
            ...
            ty.trait().data.methods.items()[i] = try self.inferFun(itm, fun_ty, ty);
          } else {
            for (ctparams, tparams) |_tvar, _typ| {
        ...
        assert(trt.data.fields.len == 0);
      }
      if (trt.trait) |trait| {
        ty.trait().data.trait = try self.resolveTraitType(trait, node.getToken(), self.al);
      }
  ...
  fn hasTrait(trait: *Type, kind: []const u8) ?*Type {
    if (trait.isTrait()) {
      if (trait.trait().isBuiltinTrait(kind)) {
        return trait;
      }
    } else {
      for (trait.union_().variants.values()) |ty| {
        if (ty.trait().isBuiltinTrait(kind)) {
          return ty;
        }
      }
    }
  ...
  fn checkBinaryOverload(self: *Self, lhs_ty: *Type, rhs_ty: *Type, optype: tir.OpType) ?*Type {
    const trait = lhs_ty.klass().data.trait.?;
    if (optype.isEqCmpOp()) {
      if (hasTrait(trait, "Eq")) |trt| {
        const tparams = trt.trait().tparams.?;
        if (tparams[0].isEitherWayRelatedTo(rhs_ty, .RCAny, self.al)) {
          return &UnitTypes.tyBool;
        }
      }
    } else if (optype.isOrdCmpOp()) {
      // <, >, <=, >=
      if (hasTrait(trait, "Ord")) |trt| {
        const tparams = trt.trait().tparams.?;
        if (tparams[0].isEitherWayRelatedTo(rhs_ty, .RCAny, self.al)) {
          return &UnitTypes.tyBool;
        }
      }
    } else if (optype.getTraitName()) |name| {
      // +, -, *, /
      if (hasTrait(trait, name)) |trt| {
        const tparams = trt.trait().tparams.?;
        if (tparams[0].isEitherWayRelatedTo(rhs_ty, .RCAny, self.al)) {
          return tparams[1];
        }
  ...
  fn checkBinary(self: *Self, node: *tir.BinaryNode, lhs_ty: *Type, rhs_ty: *Type, narrowed: bool) !*Type {
    // source is type of rhs
    // node.typ is type of lhs
    const err_token = Token.fromBinaryNode(node);
    const optype = node.optype();
    if (optype == .OpEqq or optype == .OpNeq or optype == .OpIs) {
      const rc: RelationContext = if (optype == .OpIs) .RCIs else .RCAny;
      if (!lhs_ty.isEitherWayRelatedTo(rhs_ty, rc, self.al)) {
        return self.error_(
          true, err_token,
          "Types must be related for this operation.{s}'{s}' is not related to '{s}'",
          .{
            if (!narrowed) " " else " Narrowed type ",
            self.getTypename(lhs_ty), self.getTypename(rhs_ty),
          }
        );
      }
      if (optype == .OpEqq or optype == .OpNeq) {
        const lhsty = lhs_ty.klassOrInstanceClass();
        const rhsty = rhs_ty.klassOrInstanceClass();
        if (lhsty.isClass() and lhsty.klass().hasTrait()) {
          if (self.checkBinaryOverload(lhsty, rhsty, optype)) |ty| {
            return ty;
          }
        }
      }
    ...
    if (optype == .OpAnd or optype == .OpOr) {
      if (!lhs_ty.isEitherWayRelatedTo(rhs_ty, .RCAny, self.al)) {
        return self.error_(
          true, err_token,
    ...
    assert(node.op_tkty != .TkPipeGthan);
    // .OpLess, .OpGrt, .OpLeq, .OpGeq, .OpAdd, .OpSub, .OpMul, .OpDiv, etc.
    var errty: ?*Type = null;
    if (!lhs_ty.typeidEql(&UnitTypes.tyNumber)) {
      errty = lhs_ty;
    } else if (!rhs_ty.typeidEql(&UnitTypes.tyNumber)) {
      errty = rhs_ty;
    }
    if (errty != null) {
      const lhsty = lhs_ty.klassOrInstanceClass();
      ...
      return self.error_(
        ...
        .{
          name, op, name,
          self.getTypename(lhs_ty), op, self.getTypename(rhs_ty)
        }
      );
    }
    return if (optype.isCmpOp()) &UnitTypes.tyBool else lhs_ty;
  }
      ...
      for (trait.union_().variants.values()) |trt| {
        for (trt.trait().data.methods.items()) |mth| {
          ...
          if (disamb.get(token.lexeme())) |tk| {
            self.softError(token, "conflicting trait method '{s}'", .{token.lexeme()});
            self.softErrorFmt(tk, 4, 2, "Method already specified here:", .{});
          } else {
  ...
  fn checkTraitChainForDuplicateMethods(
    self: *Self, trait: *Type,
    disamb_name: *ds.StringHashMap(Token),
    disamb_type: *std.AutoHashMap(*Type, u32),
  ) !void {
    if (disamb_type.get(trait) != null) return;
    disamb_type.put(trait, 1) catch {};
    if (trait.isTrait()) {
      if (trait.trait().data.node) |node| {
        for (node.NdTrait.data.methods.items()) |nd| {
          const tk = nd.getBasicFun().data.name.?;
          if (disamb_name.get(tk.lexeme())) |found| {
            self.softError(found, "conflicting trait method '{s}'", .{found.lexeme()});
            return self.errorFmt(tk, 4, 2, "Method already specified here:", .{});
          }
          disamb_name.set(tk.lexeme(), tk);
        }
      }
      if (trait.trait().data.trait) |trt| {
        try self.checkTraitChainForDuplicateMethods(trt, disamb_name, disamb_type);
      }
    } else if (trait.isUnion()) {
      for (trait.union_().variants.values()) |ty| {
        try self.checkTraitChainForDuplicateMethods(ty, disamb_name, disamb_type);
      }
    }
  }

  fn checkTrait(self: *Self, ty: *Type, trait: *Type, debug: Token) TypeCheckError!void {
    var disamb_name = ds.StringHashMap(Token).init(self.al);
    var disamb_type = std.AutoHashMap(*Type, u32).init(self.al);
    try self.checkTraitChainForDuplicateMethods(trait, &disamb_name, &disamb_type);
    self.missing_tmtds.clearRetainingCapacity();
    var err = false;
    self._checkTrait(ty.klassOrInstanceClass(), trait, debug) catch {
      err = true;
    };
  ...
  fn infer(self: *Self, node: *Node) TypeCheckError!*Type {
    return switch (node.*) {
      .NdPubVarDecl => |*nd| {
        const ty = try self.infer(nd.decl);
        self.cmo.setIdTy(nd.getVarDecl().name.lexeme(), ty, .SpecPublic);
        return ty;
      },
      .NdConstVarDecl => |*nd| {
        var ty = try self.inferVarDecl(nd, true);
        var res = ty.forceClone(self.al);
        res.mutable = false;
        self.insertVar(nd.name.lexeme(), res);
        return res;
      },
      .NdVarDecl => |*nd| self.inferVarDecl(nd, false),
      ...
      .NdBasicFun => self.inferFun(node, null, null),
  ...
  fn buildFunFlow(self: *Self, node: *Node) !*FlowGraph {
    ...
    const graph = self.builder.buildFun(node);
  ...
  fn setupEntryModule(self: *Self, root: *Node, display_diag: bool) !void {
  ...
  pub fn typecheck(self: *Self, root: *Node, display_diag: bool) TypeCheckError!void {
    ...
    self.linker.tc = self;
    self.loadPrelude();
    try self.setupEntryModule(root, display_diag);
    self.flowInferProgram(self.cunit, self.cmt);
    if (self.diag.hasErrors()) {
      return error.CheckError;
    }
  }
